    ...
    <div className={`m-2`}>
      ...
      <div>
        <div className={`mt-4 flex space-x-2`}>
          <JiraLink ticketDescription={ticketDescription} />
          <MRLink ticketDescription={ticketDescription} />
        </div>

        <PhotonLinks selectedDomain={selectedDomain} selectedJiraTicket={selectedJiraTicket} />

        <pre className={`mt-48`}>{JSON.stringify(ticketDescription, null, 2)}</pre>
      ...
      <Head>
        <title>{`PHO-${ticketDescription?.ticketNumber} | ${ticketDescription?.description} | ${appTitle}`}</title>
      </Head>
    </div>
  )
}
