...
import { pathHierarchy, environmentDomains, ephemeralBaseDomain } from '@/constants'
...
export default function PhotonLinks(props) {
  ...
  const derivedDomains = {
    ticket: buildDomain(`pho-${selectedJiraTicket.value}`),
    rc: buildDomain('rc-' + selectedJiraTicket.rc.replaceAll('.', '-')),
  }

  function domainLookup(key) {
    return environmentDomains[key] || derivedDomains[key]
  }
        ...
        {data.map((item) => (
          <>
            {Array.isArray(item) ? (
              ...
              <div>
                <a href={`${domainLookup(selectedDomain.value)}${item.path}`}>{item.name}</a>
              </div>
            )}
          </>
        ))}
